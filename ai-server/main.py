...
#
# 가중치 근거 — 맥미니 M1(CPU), 30초 영상 2개, stride=2, 총 약 120초 실측:
#   다운로드 5초 / 기준 추출 46초 / 연습 추출 46초 / 정렬·피드백 5초 / 이미지 18초
# (stride=1일 때는 추출이 각 89초, 총 201초였다. engine/analyze.py의 실측표 참조)
# 영상이 길어지면 절대 시간은 늘지만 비율은 대체로 유지된다(추출이 지배적).
PROGRESS_STAGES = [
    ('download',          '영상 받는 중',           0.04),
    ('extract_reference', '기준 영상 분석 중',       0.38),
    ('extract_practice',  '내 영상 분석 중',         0.38),
    ('align',             '동작 맞추는 중',          0.05),
    ('render',            '비교 이미지 만드는 중',    0.15),
]
