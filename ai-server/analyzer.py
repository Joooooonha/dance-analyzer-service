"""[사용 안 함 — v3.0에서 engine/analyze.py로 대체됨]

이 파일은 참고용으로만 남겨둔다. `main.py`는 더 이상 import하지 않는다.
대체된 이유(측정 근거는 model-research/TASKS.md):

  - 결측 관절을 `similarity = 1.0`(완벽 일치)로 처리 → 가려진 관절이 많을수록
    점수가 올라가는 역전이 있었다.
  - 좌표 코사인 유사도 비교 → 카메라 거리·화면 내 위치 차이가 동작 차이로
    오인된다. 각도 비교로 교체.
  - fastdtw에 밴드 제약이 없어 DTW가 멀리 떨어진 "포즈가 비슷한" 지점으로
    도망갈 수 있었다.
  - 0~100 점수의 환산 상수가 임의값이라 절대값에 의미가 없었다.
"""
"""
