# src/dtw_compare_fastdtw.py
#
# [DEPRECATED] 사용하지 않는다. `src/dtw_compare.py`(dtaidistance dtw_ndim)를 쓸 것.
#
# 폐기 이유 (TASKS.md D7):
#   fastdtw는 근사 알고리즘이라 최적 경로를 보장하지 않고, 실무적인 데이터
#   크기에서는 밴드 제약을 건 정확한 DTW보다 오히려 느리다
#   (Wu & Keogh, "FastDTW is approximate and Generally Slower than the
#    Algorithm it Approximates", IEEE TKDE).
#   또한 이 파일의 구현은 좌표를 0으로 패딩해서 "가려진 관절끼리 거리 0 =
#   완벽 일치"가 되는 문제를 그대로 갖고 있다.
#
# 비교 기록으로만 남겨둔다.
import numpy as np
